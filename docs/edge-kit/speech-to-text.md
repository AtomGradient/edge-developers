...
Use `EdgeVoice` for microphone recording. Use `STTEngine` from `EdgeInference` for native ASR in builds that include the speech runtime. `WhisperEngine` is currently a preview bridge for future whisper.cpp integration; in `edge-kit@1.0.0-rc103` it is a skeleton and does not perform real transcription.
...
- [Text to speech](/docs/edge-kit/text-to-speech) — Complete the voice loop.

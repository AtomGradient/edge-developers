...
:::info 前置条件
完成 [Build your first device Agent](/docs/get-started/minute-demo) 中的 CLI 学习 demo。你需要已安装 Edge Studio 和本地模型。
:::
...
{/* TODO: 截图 — Edge Studio 导出界面，Export → Edge Scaffold 按钮 */}
:::note 📸 截图占位
...
1. **设备评估** — App 检测你的设备硬件（芯片、内存、存储），计算 AI IQ 分数
2. **模型选择** — 根据设备能力推荐模型 tier
3. **模型下载** — 下载模型到本地（首次需要网络，之后不需要）

{/* TODO: 截图 — Onboarding 设备评估页面，显示 AI IQ 分数 */}
:::note 📸 截图占位
...
模型加载完成后，进入 Chat 页面。先问一个普通问题，确认推理正常：

```text
这个月账单付完还剩 800 块，我该怎么用？
```

你应该看到一个**通用的回答**——储蓄、投资、还债等建议，没有针对你的偏好。

这是 **base model 的状态**：模型可以聊天，但不了解你。

{/* TODO: 截图 — Chat 界面，base model 给出的通用理财建议 */}
:::note 📸 截图占位
Base model 通用回答
:::
...
{/* TODO: 截图 — Settings → Personalization → Domain 选择器，选中 finance */}
:::note 📸 截图占位
Domain 选择器界面
:::
...
{/* TODO: 截图 — Sample Data 页面，显示已加载的合成消费记录列表 */}
:::note 📸 截图占位
加载后的合成消费记录
:::
...
{/* TODO: 截图 — Personalization 页面的数据管线状态 */}
:::note 📸 截图占位
Data Pipeline 状态面板
:::
...
当足够的本地信号（facts + corrections）积累后，App 可以触发 Edge Halo profile job。
...
{/* TODO: 截图 — 学习过程中的进度指示（如果有），或 Neural Imprint 状态从 "Not active" 变为 "Active" */}
:::note 📸 截图占位
Neural Imprint 状态变为 Active
:::

在 **Personalization** 页面顶部的 **Persona Runtime** 区域，你应该看到：

- **Neural Imprint**: Active ✅
- Artifact SHA256（前 12 位哈希）
- Cache Backend

这就是设备端学习的证据：NI 产物在 iPhone 上本地生成并恢复。
...
现在的回答应该更**保守、关注现金流**——和你在 CLI demo 中看到的行为变化一致。
...
**这就是端侧学习的 aha moment。** 同一个模型，同一个问题，设备本地学习后回答变了。不是 prompt stuffing——没有把偏好塞进 prompt。不是 LoRA——没有重新训练模型。NI 产物在设备上本地生成、本地恢复。

{/* TODO: 截图 — 学习后的 Chat 回答，明显更保守 */}
:::note 📸 截图占位
学习后回答（保守、现金流导向）
:::
...
回答应该仍然正常——保守风格、关注现金流、不编造收益数据。

**这证明了：模型推理 + 学习状态恢复，全程不需要网络。** 所有计算都在设备本地完成。

{/* TODO: 截图 — 飞行模式下的对话，右上角显示飞行模式图标 */}
:::note 📸 截图占位
飞行模式下的正常对话
:::
...
**学习状态是可移除的。** 用户可以随时清除，模型回到出厂状态。这是 Edge 的承诺：可恢复、可移除、用户可控。

{/* TODO: 截图 — 清除后 Neural Imprint 显示 "Not active" */}
:::note 📸 截图占位
...
你刚刚用 finance domain 体验了完整的设备端学习循环。现在回到 **Settings → Personalization**，切换到其他 domain 试试：

- **health** — 加载健身记录，看看助手是否学会了你的运动偏好
- **cooking** — 加载口味数据，看看助手推荐菜谱是否变了
- **journal** — 加载日记数据，看看助手的写作建议风格是否变了

每个 domain 有独立的合成数据集，学习循环相同：加载数据 → 触发学习 → NI 生成 → restore → 行为变化。
...
| 路径 | 你证明了什么 |
|------|-------------|
| [CLI demo](/docs/get-started/minute-demo) | 学习机制有效：inspect → before → learn → after |
| **本页** | 设备端体验真实：iPhone 上加载、学习、恢复、离线、可移除 |

这就是 Edge 的完整故事：

> **设备是 Agent，App 是载体。**
> 模型在设备上学习用户偏好、学习使用本地工具。
> 学习状态本地生成、本地恢复、可跨设备迁移、可随时移除。
> 数据从不离开设备。基础模型包从不被替换。
...
| 目标 | 去哪里 |
|------|--------|
| 了解 Neural Imprint 和 LoRA/SFT 的区别 | [Neural Imprint vs LoRA](/docs/guides/neural-imprint-vs-lora) |

...
[Install Edge Studio](/docs/quickstart/install).
...
Imprint (see [Custom Python Tools](/docs/knowledge-tools/custom-python-tools)). External
...
**→ [Device Agent Learning](/docs/labs/device-learning-iphone)** — export the carrier, deploy to iPhone, pick a domain, trigger on-device learning, see the behavior change, verify offline, and remove the learned state.
...
- Install reference: [Install Edge Studio](/docs/quickstart/install)

...
> **⚠️ Do not run `xcodegen generate` on the exported project.** The export wires the model in as an on-demand resource (ODR) by post-processing `project.pbxproj` — that wiring is **not** in `project.yml`, so `xcodegen generate` regenerates the project and silently drops it. If you need to change `project.yml`, re-export from Edge Studio so the model resource stays wired.
...
# 2. Signed build for that device. Replace <DEVICE_ID> and <TEAM_ID>.

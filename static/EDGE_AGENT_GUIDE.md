...
> **Editing `project.yml` later?** The export already generated the `.xcodeproj`, so you normally don't need to run `xcodegen generate`. On **rc15+** it is safe to regenerate — the export writes the model's on-demand-resource (ODR) wiring (`KnownAssetTags` + the model source's `resourceTags`) into `project.yml`, so `xcodegen generate` preserves it. (On rc14 and earlier the ODR lived only in the generated `.xcodeproj` and regenerating would silently drop it — use rc15+.)

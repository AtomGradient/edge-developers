...
使用 `EdgeVoice` 进行麦克风录音。对于包含语音运行时的构建，使用 `EdgeInference` 中的 `STTEngine` 做原生 ASR。`WhisperEngine` 目前只是未来 whisper.cpp integration 的 预览桥接；在 `edge-kit@1.0.0-rc98` 中它仍是 skeleton，不执行真实转写。

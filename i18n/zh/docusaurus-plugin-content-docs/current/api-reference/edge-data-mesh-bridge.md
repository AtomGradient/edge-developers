...
这个模块只是 generic glue。App-specific event schemas、correction text、privacy policy 和业务规则仍由 app 自己拥有。
:::
...
| `init(store:)` | 使用 app-provided `EventStore`。 |
| `collectTrainingSample(appId:eventType:payload:tags:)` | 向 mesh event store 追加 generic training event。 |
...
只传入 app 已经判定为适合目标 sink 的 payload。遵守 `Sensitivity`，不要在 logs、collaboration messages 或 exported receipts 中写入 raw correction text。

## 安全边界

- 不要把 app business fields 或 domain-specific interpretation 放进这个 bridge。
- Data retention、consent 和 redaction policy 留在 app 内。

...
Neural Imprint 是 Edge 的端侧学习契约。它让 app 保持稳定的基础模型包，同时把用户特定学习状态作为本地、可删除产物恢复进运行时。
...
- **基础模型包保持不变。** 个性化不会替换已发布模型，也不会修改模型权重。
...
- **不重放 prompt。** 用户学习状态不会被粘贴进每一次请求；generation 继续聚焦当前消息和工具上下文。
...
本页描述产品和集成契约。它不公开私有 artifact 构造方法、训练内部机制、runtime 公式或 kernel 实现细节。

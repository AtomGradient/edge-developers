...
这是 [构建第一个设备 Agent](/docs/quickstart/first-agent) 使用的同一条理财信号。
...
- 如果还没有比较学习前/学习后的回答，先运行 [设备 Agent 演示](/docs/quickstart/first-agent)。
- 阅读 [Edge Scaffold 配置](/docs/studio/scaffold)，了解 Edge Studio 导出时会重写哪些模板字段。
- 选择本地学习产物还是模型 adapter release 时，阅读 [Neural Imprint vs LoRA](/docs/concepts/neural-imprint-vs-lora)。

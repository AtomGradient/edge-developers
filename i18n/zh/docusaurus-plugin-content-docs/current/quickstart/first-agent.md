...
`edge doctor` 会检查 Python 环境、模型路径和系统兼容性。源码安装见 [安装 Edge Studio](/docs/quickstart/install)，但这条路径不需要源码安装。
...
[自定义 Python 工具](/docs/knowledge-tools/custom-python-tools)）。外部 provider 进程仍是后续
独立契约。
...
- 源码安装参考：[安装 Edge Studio](/docs/quickstart/install)

...
基础对话跑通后，继续看 [构建第一个设备 Agent](/docs/quickstart/first-agent)，检查合成理财信号，并看到用户特定学习在不创建新模型 release 的情况下被恢复。然后把同一套生命周期导出进一个 [Agent 载体](/docs/quickstart/build-agent-carrier)。

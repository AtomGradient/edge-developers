...
开发者预览阶段，Swift 包使用 `1.0.0-rcN` 标签发布。Edge Studio 的 Python 包当前使用 PyPI 版本 `0.0.1rc7` 和 GitHub tag `v0.0.1rc7`。破坏性变更会在这里记录迁移步骤。正式可用后会遵循语义化版本。
...
| 产品表面 | 当前访问方式 | 说明 |
|---|---|---|
| Edge Studio | Python 软件包 `edge-studio==0.0.1rc7`，GitHub tag `v0.0.1rc7` | 安装后只暴露一个 `edge` 命令。用 `edge studio` 启动本地 Studio UI。 |
